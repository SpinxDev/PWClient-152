...
void CECIvtrEquip::SetNewMark(const ACString &strMark, A3DCOLOR clr)
{
	...
	{
		//	转换特殊字符
		m_strMaker = CECGame::AUI_ConvertHintString(m_strMaker);

		//	转换屏蔽字符
		g_pGame->FilterBadWords(m_strMaker);
...
DWORD CECIvtrEquip::PropEffectEssence()
{
	...
	{
		CECGame::ItemExtPropTable::pair_type Pair = PropTab.get(m_aProps[i].iType);
...
void CECIvtrEquip::AddReputationReqDesc()
{
	...
	{
		CECStringTab* pDescTab = g_pGame->GetItemDesc();
		AddDescText(ITEMDESC_COL_WHITE, true, pDescTab->GetWideString(ITEMDESC_REPUTATION_REQ), m_iReputationReq);
	}
}
