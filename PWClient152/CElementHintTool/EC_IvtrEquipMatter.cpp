...
const wchar_t* CECIvtrForceToken::GetNormalDesc(bool bRepair)
{
	...
	elementdataman* pDB = g_pGame->GetElementDataMan();
	DATA_TYPE DataType;
	const FORCE_CONFIG* pConfig = !m_Essence.require_force ? NULL: (const FORCE_CONFIG *)pDB->get_data_ptr(m_Essence.require_force, ID_SPACE_CONFIG, DataType);
	if(pConfig)
...
// {
// 	float ratio_suite(1.f), ratio_profession(1.f), ratio_rebirth(1.f);
// 	DATA_TYPE DataType = DT_INVALID;
// 	elementdataman* pDataMan = g_pGame->GetElementDataMan();
...
// 			if (iItemCnt == max_equips) ratio_suite = 1.f + pSuiteEss->promote_ratio;
// 		}
// 	}
// 	ratio_profession = pHostPlayer->GetCharacterClassConfig()->poker_adjust[m_Essence.type];
// 	switch(m_Essence.rebirth_times) {
// 	case 1: ratio_rebirth = 1.3f; break;
// 	case 2: ratio_rebirth = 1.6f; break;
// 	default: ratio_rebirth = 1.f; break;
// 	}
// 	
...
// 	m_strDesc += _AL(" ");
// 	AddDescText(white, true, g_pGame->GetProfName(pHost->GetProfession()));
	...
	// 统御力要求
	if (m_Essence.require_leadership) {
		col = white;
...
int CECIvtrGeneralCard::GetItemLevel() const
{
	return m_Essence.rank;
}
