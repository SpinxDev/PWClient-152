/*
 * FILE: A3DAutoEagle.cpp
 *
 * DESCRIPTION: The implementation A3DAutoEagle class
 *
...
bool A3DAutoEagle::Init (A3DEngine *pEngine)
{
	Reset();
...
void A3DAutoEagle::Update (DWORD dwDeltaTime, ActionList_e action /*= ACTION_FIRST*/)
{
	...
	if(!m_bDoingAction)
	{
		if( m_bUseECModel)
		{
			...
			{
				m_pECModel->PlayActionByName("…»∂Ø≥·∞Ú", 1.0f);
			}
		}
		else
		{
			if( action == ACTION_FIRST)
			{
				m_pSkinModel->PlayActionByNameDC("…»∂Ø≥·∞Ú", -1);
			}
		}
...
void A3DAutoEagle::ApplySteeringForce (float fElapsedTime)
{
	float rate = 0.21;
...
A3DVECTOR3 A3DAutoEagle::RespondToBoundary (const A3DVECTOR3& vCenter, float fBoundaryRadius)
{
...
void A3DAutoEagle::ClampPosition (A3DTerrain2 * pTerrain, A3DTerrainWater * pTerrainWater, DWORD dwDeltaTime)
{
...
void A3DAutoEagle::ClampAboveTerrain (A3DVECTOR3 pos, A3DTerrain2* pTerrain)
{
	// hard boundary
	A3DVECTOR3 vel = GetVelocity();
...
void A3DAutoEagle::ClampAboveTerrainWater (A3DVECTOR3 pos, A3DTerrainWater* pTerrainWater)
{
	// hard boundary
	A3DVECTOR3 vel = GetVelocity();
	float speed = vel.Magnitude();
...
A3DVECTOR3 A3DAutoEagle::SteerForWander (float dt)
{
	float speed = 16 * dt;
...
A3DVECTOR3 A3DAutoEagle::SteerForSeek (const A3DVECTOR3& target)
{
    A3DVECTOR3 desiredVelocity = target - GetPosition();
    return desiredVelocity - GetVelocity();
}

A3DVECTOR3 A3DAutoEagle::SteerForFlee (const A3DVECTOR3& target)
{
...
bool A3DAutoEagle::IsInBoidNeighborhood (const A3DAutonomousAgent* other, float minDistance, float maxDistance, float cosMaxAngle)
{
...
A3DVECTOR3 A3DAutoEagle::SteerForSeparation (float maxDistance, float cosMaxAngle, const A3DAutonomousGroup* pGroup)
{
...
A3DVECTOR3 A3DAutoEagle::SteerForAlignment (float maxDistance, float cosMaxAngle, const A3DAutonomousGroup* pGroup)
{
    A3DVECTOR3 steering(0, 0, 0);
    int neighbors = 0;
...
A3DVECTOR3 A3DAutoEagle::SteerForCohesion (float maxDistance, float cosMaxAngle, const A3DAutonomousGroup* pGroup)
{
...
void A3DAutoEagle::RegenerateLocalSpace (A3DVECTOR3 vNewForward, float fBankAngle)
{
