/*
 * FILE: A3DAutoFirefly.cpp
 *
 * DESCRIPTION: The implementation A3DAutoFirefly class
 *
...
void A3DAutoFirefly::Reset ()
{
	...
	m_fBankAngle = 0;
	m_fBankAngleMax = 0.001;
	m_bDoingAction	= false;
}

bool A3DAutoFirefly::Init (A3DEngine *pEngine)
{
	Reset();
...
void A3DAutoFirefly::Update (DWORD dwDeltaTime, ActionList_e action/* = ACTION_FIRST*/)
{
	...
	if(vel.MagnitudeH()<0.1)
		vel = GetForward();
	else
		vel.y = 0;

	RegenerateLocalSpace(vel, 0);
	float animrate = GetSpeed() * 2;
...
A3DVECTOR3 A3DAutoFirefly::RespondToBoundary (const A3DVECTOR3& vCenter, float fBoundaryRadius)
{
...
void A3DAutoFirefly::ClampPosition (A3DTerrain2 * pTerrain, A3DTerrainWater * pTerrainWater, DWORD dwDeltaTime)
{
...
void A3DAutoFirefly::ClampAboveTerrain (A3DVECTOR3 pos, A3DTerrain2* pTerrain)
{
	// hard boundary
	A3DVECTOR3 vel = GetVelocity();
...
void A3DAutoFirefly::ClampAboveTerrainWater (A3DVECTOR3 pos, A3DTerrainWater* pTerrainWater)
{
	// hard boundary
	A3DVECTOR3 vel = GetVelocity();
	float speed = vel.Magnitude();
...
void A3DAutoFirefly::ClampBelowTerrainHeight (A3DVECTOR3 pos, A3DTerrain2* pTerrain, float fHeight)
{
	// hard boundary
	A3DVECTOR3 vel = GetVelocity();
	float speed = vel.Magnitude();
...
A3DVECTOR3 A3DAutoFirefly::SteerForWander (float fDeltaTime)
{
...
A3DVECTOR3 A3DAutoFirefly::SteerForSeek (const A3DVECTOR3& target)
{
...
A3DVECTOR3 A3DAutoFirefly::SteerForFlee (const A3DVECTOR3& target)
{
...
bool A3DAutoFirefly::IsInBoidNeighborhood (const A3DAutonomousAgent* other, float minDistance, float maxDistance, float cosMaxAngle)
{
...
A3DVECTOR3 A3DAutoFirefly::SteerForSeparation (float maxDistance, float cosMaxAngle, const A3DAutonomousGroup* pGroup)
{
...
A3DVECTOR3 A3DAutoFirefly::SteerForAlignment (float maxDistance, float cosMaxAngle, const A3DAutonomousGroup* pGroup)
{
    A3DVECTOR3 steering(0, 0, 0);
    int neighbors = 0;
...
A3DVECTOR3 A3DAutoFirefly::SteerForCohesion (float maxDistance, float cosMaxAngle, const A3DAutonomousGroup* pGroup)
{
...
void A3DAutoFirefly::ApplySteeringForce (float fElapsedTime)	// time in sec
{
	float rate = 0.5;
	...
	// add turbulence
	m_vVelocity += 
		A3DVECTOR3((frandom01()-0.5f)*0.05, (frandom01()-0.5f)*0.05, (frandom01()-0.5f)*0.05)
		*m_fSpeedMax;
