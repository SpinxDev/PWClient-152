 ...
 */
void CDecoder::init_layer3(int down_sample_sblimit)
{
  int i,j,k,l;

  for(i=-256;i<118+4;i++)
    gainpow2[i+256] = (float)(pow((double)2.0,-0.25 * (double) (i+210) ));
  ...
  for(i=0;i<18;i++) {
    win[0][i]    = win[1][i]    = (float)(0.5 * sin( M_PI / 72.0 * (double) (2*(i+0) +1) ) / cos ( M_PI * (double) (2*(i+0) +19) / 72.0 ));
    win[0][i+18] = win[3][i+18] = (float)(0.5 * sin( M_PI / 72.0 * (double) (2*(i+18)+1) ) / cos ( M_PI * (double) (2*(i+18)+19) / 72.0 ));
  }
  for(i=0;i<6;i++) {
  ...
  for(i=0;i<12;i++) {
    ...
    for(j=0;j<6;j++)
      COS1[i][j] = (float)(cos( M_PI / 24.0 * (double) ((2*i+7)*(2*j+1)) ));
  }
 ...
 */
 void CDecoder::III_get_side_info(struct III_sideinfo *si,int stereo,
 int ms_stereo,long sfreq,int single,int lsf)
{
   ...
   si->main_data_begin = m_readers->getbits(tab[1]);
   if (stereo == 1)
     si->private_bits = m_readers->getbits_fast(tab[2]);
   else 
     si->private_bits = m_readers->getbits_fast(tab[3]);

   if(!lsf) {
     for (ch=0; ch<stereo; ch++) {
         si->ch[ch].gr[0].scfsi = -1;
         si->ch[ch].gr[1].scfsi = m_readers->getbits_fast(4);
     }
   }
   ...
   for (gr=0; gr<tab[0]; gr++) {
     for (ch=0; ch<stereo; ch++) {
       ...
       gr_info->part2_3_length = m_readers->getbits(12);
       gr_info->big_values = m_readers->getbits(9);
       if(gr_info->big_values > 288) {
          fprintf(stderr,"big_values too large!\n");
          gr_info->big_values = 288;
       }
       gr_info->pow2gain = gainpow2+256 - m_readers->getbits_fast(8) + powdiff;
       if(ms_stereo)
         gr_info->pow2gain += 2;
       gr_info->scalefac_compress = m_readers->getbits(tab[4]);

       if(m_readers->get1bit()) { /* window switch flag  */
         int i;
         gr_info->block_type       = m_readers->getbits_fast(2);
         gr_info->mixed_block_flag = m_readers->get1bit();
         gr_info->table_select[0]  = m_readers->getbits_fast(5);
         gr_info->table_select[1]  = m_readers->getbits_fast(5);
         /*
         ...
         for(i=0;i<3;i++)
           gr_info->full_gain[i] = gr_info->pow2gain + (m_readers->getbits_fast(3)<<3);
...
/* check this again for 2.5 and sfreq=8 */
         ...
         int i,r0c,r1c;
         for (i=0; i<3; i++)
           gr_info->table_select[i] = m_readers->getbits_fast(5);
         r0c = m_readers->getbits_fast(4);
         r1c = m_readers->getbits_fast(3);
       ...
       if(!lsf)
         gr_info->preflag = m_readers->get1bit();
       gr_info->scalefac_scale = m_readers->get1bit();
       gr_info->count1table_select = m_readers->get1bit();
     }
   }
}
 ...
 */
void CDecoder::III_get_side_info_2(struct III_sideinfo *si,int stereo, int ms_stereo,long sfreq,int single)
{
   int ch;
   int powdiff = (single == 3) ? 4 : 0;

   si->main_data_begin = m_readers->getbits(8);
   if (stereo == 1)
     si->private_bits = m_readers->get1bit();
   else 
     si->private_bits = m_readers->getbits_fast(2);
 ...
 * read scalefactors
 */
int CDecoder::III_get_scale_factors_1(int *scf,struct gr_info_s *gr_info,int ch,int gr)
{
   ...
   int num1 = slen[1][gr_info->scalefac_compress];
    ...
    if (gr_info->block_type == 2) {
      ...
      if (gr_info->mixed_block_flag) {
         for (i=8;i;i--)
           *scf++ = m_readers->getbits_fast(num0);
         i = 9;
      ...
      for (;i;i--)
        *scf++ = m_readers->getbits_fast(num0);
      for (i = 18; i; i--)
        *scf++ = m_readers->getbits_fast(num1);
...
int CDecoder::III_get_scale_factors_2(int *scf,struct gr_info_s *gr_info,int i_stereo)
{
  unsigned char *pnt;
  ...
  for(i=0;i<4;i++) {
    ...
    if(num) {
      for(j=0;j<(int)(pnt[i]);j++)
        *scf++ = m_readers->getbits_fast(num);
      numbits += pnt[i] * num;
    }
    else {
      for(j=0;j<(int)(pnt[i]);j++)
...
#define REFRESH_MASK \
  while(num < BITSHIFT) { \
    mask |= m_readers->getbyte()<<(BITSHIFT-num); \
    num += 8; \
    part2remain -= 8; }

int CDecoder::III_dequantize_sample(real xr[SBLIMIT][SSLIMIT],int *scf,
   struct gr_info_s *gr_info,int sfreq,int part2bits)
{
  ...
  int num=m_readers->getbitoffset();
  long mask = (long) m_readers->getbits(num)<<(BITSHIFT+8-num);
  part2remain -= num;
 ...
 */
  ...
  while( part2remain > 16 ) {
    m_readers->getbits(16); /* Dismiss stuffing Bits */
    part2remain -= 16;
  }
  if(part2remain > 0)
    m_readers->getbits(part2remain);
 ...
 */
 void CDecoder::III_i_stereo(real xr_buf[2][SBLIMIT][SSLIMIT],int *scalefac,
...
void CDecoder::III_antialias(real xr[SBLIMIT][SSLIMIT],struct gr_info_s *gr_info) {
   int sblim;
...
void CDecoder::dct36(real *inbuf,real *o1,real *o2,real *wintab,real *tsbuf)
{
#ifdef NEW_DCT9
  real tmp[18];
#endif
 ...
 * new DCT12
 */
void CDecoder::dct12(real *in,real *rawout1,real *rawout2,register real *wi,register real *ts)
{
#define DCT12_PART1 \
             in5 = in[5*3];  \
     in5 += (in4 = in[4*3]); \
 ...
 * III_hybrid
 */
 void CDecoder::III_hybrid(real fsIn[SBLIMIT][SSLIMIT],real tsOut[SSLIMIT][SBLIMIT],
 ...
 * main layer3 handler
 */
 int CDecoder::do_layer3(struct frame *fr,int outmode,CMpg123* player)
{
  int gr, ch, ss,clip=0;
  ...
  player->GetReaders()->set_pointer(sideinfo.main_data_begin);
  ...
  for (gr=0;gr<granules;gr++) {
    ...
    for(ss=0;ss<SSLIMIT;ss++)
	{
		if(single >= 0) 
		{
			clip += (this->*fr->synth_mono)(hybridOut[0][ss],player->pcm_sample,&player->pcm_point);
		}
		else
		{
			int p1 = player->pcm_point;
			clip += (this->*fr->synth)(hybridOut[0][ss],0,player->pcm_sample,&p1);
			clip += (this->*fr->synth)(hybridOut[1][ss],1,player->pcm_sample,&player->pcm_point);
		}
