...
CObstructOperation::CObstructOperation()
{
	m_pData = 0;
	m_bStartPaint = false;
...
bool CObstructOperation::OnLButtonDown(int x, int y, DWORD dwFlags)
{
	   ...
	   m_nOperationType == OBSTRUCT_YELLOW ) )
	{
		...
		if(m_nOperationType == OBSTRUCT_YELLOW)
		{
			...
			if(pRWnd->GetActiveViewport() != VIEW_XZ)
			{
				...
				SAMPLE_POS mapPos;
				pMap->GetPassMapGen()->Transf3DTo2D(vecHitPos,mapPos);
				...
				}else
				{
					...
					pMap->GetPassMapGen()->Transf2DTo3D(mapPos,vPixelCenter);
					mapPos.h = vecHitPos.y - pTerrain->GetPosHeight(vPixelCenter);
				}
				
				pMap->GetPassMapGen()->SetPosReachable(mapPos);
...
bool CObstructOperation::OnLButtonUp(int x,int y, DWORD dwFlags)
{
	m_bStartPaint = false;
	...
	if(pMap)
	{
		...
		if(m_nOperationType == OBSTRUCT_CH_CANNOT_PASS)
		{
			// set Convex hull's passable property
			A3DVECTOR3 vecHitPos,vecNormal;
			CConvexHullData *pTraceCHData = NULL;
			float fFraction = 1.0f;
			if(pMan->RayTraceStaticModel(vStart,vPos - vStart,vecHitPos,&fFraction,vecNormal,pTraceCHData))
			{
				if(pTraceCHData)
				{
					DWORD dwFlags = pTraceCHData->GetFlags();
					
					if(dwFlags&CHDATA_NPC_CANNOT_PASS)
						dwFlags &=~(CHDATA_NPC_CANNOT_PASS);
					else
						dwFlags |=CHDATA_NPC_CANNOT_PASS;
					
					pTraceCHData->SetFlags(dwFlags);					
					pMap->SetModifiedFlag(true);
				}
			}
			return true;	
		}
		else if( m_nOperationType == OBSTRUCT_SET_SEED)
		{
			...
			SAMPLE_POS seedpos;
			pMap->GetPassMapGen()->Transf3DTo2D(vecHitPos,seedpos);
			pMap->GetPassMapGen()->ExpandPassRegion(seedpos);
			pMap->SetModifiedFlag(true);
			
			m_vHitPos = vecHitPos;
			return true;
		}else if(m_nOperationType == OBSTRUCT_CH_EDIT)
		{
			// set Convex hull's passable property
			A3DVECTOR3 vecHitPos,vecNormal;
			CConvexHullData *pTraceCHData = NULL;
			...
			if(pMan->RayTraceStaticModel(vStart,vPos - vStart,vecHitPos,&fFraction,vecNormal,pTraceCHData))
			{
				...
				{
					DWORD dwFlags = pTraceCHData->GetFlags();
					CHullPropertyDlg dlg;
					if(dwFlags & CHDATA_SKIP_COLLISION)
						dlg.m_bC = true;
					else dlg.m_bC = false;
					
					if(dwFlags & CHDATA_SKIP_RAYTRACE)
						dlg.m_bT = true;
					else dlg.m_bT = false;
					
					if(IDOK==dlg.DoModal())
					{
						dwFlags = pTraceCHData->GetFlags();
						if(dlg.m_bC) dwFlags |= CHDATA_SKIP_COLLISION;
						else dwFlags &= ~CHDATA_SKIP_COLLISION;
						
						if(dlg.m_bT) dwFlags |= CHDATA_SKIP_RAYTRACE;
						else dwFlags &= ~CHDATA_SKIP_RAYTRACE;
						pTraceCHData->SetFlags(dwFlags);
						pMap->SetModifiedFlag(true);
					}
				}
...
bool CObstructOperation::OnMouseMove(int x,int y,DWORD dwFlags)
{
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
	if(pMap &&
	   (m_nOperationType == OBSTRUCT_RED ||
...
void CObstructOperation::UpdateObsData(bool bReached)
{
	...
	if(pMap)
	{
		...
		for(int i=0; i<NumZ; i++)
		{
			...
			for(int n=0; n<NumX; n++)
			{
				...
				SAMPLE_POS vsc;
				pMap->GetPassMapGen()->Transf3DTo2D(vpos,vsc);
				a_Clamp(vsc.u, 0, 1024);
				a_Clamp(vsc.v, 0, 1024);
				//////////////////////////////////////////////////////////////////////////
				// in new SetPosReachable method, vsc.h should be the delta height with terrain!
				if(bReached) pMap->GetPassMapGen()->SetPosReachable(vsc);
				else pMap->GetPassMapGen()->SetPosUnreachable(vsc);
				pMap->SetModifiedFlag(true);
			}
		}
...
void CObstructOperation::UpdateBldObsData(A3DVECTOR3& vStart, A3DVECTOR3& vEnd, bool bReached)
{
	float m_fRadius = g_Configs.iBrushRadius;
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
	if(pMap)
	{
		...
		for(int i=0; i<NumZ; i++)
		{
			...
			for(int n=0; n<NumX; n++)
			{
				...
				SAMPLE_POS mapPos;
				pMap->GetPassMapGen()->Transf3DTo2D(vecHitPos,mapPos);
				if(bReached) pMap->GetPassMapGen()->SetPosReachable(mapPos);
				else pMap->GetPassMapGen()->SetPosUnreachable(mapPos);
				pMap->SetModifiedFlag(true);
			}
		}
	}
...
void CObstructOperation::Render(A3DViewport* pA3DViewport)
{
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
...
void CObstructOperation::Init(int type)
{
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
	if(pMap == NULL) return;
...
void CObstructOperation::SaveObsToFile(CString strPathName)
{
...
void CObstructOperation::DrawGrid(A3DVECTOR3 v1, A3DVECTOR3 v2, A3DVECTOR3 v3, A3DVECTOR3 v4,int nSel, int DeltaHeight)
{
	DWORD clr;
...
void CObstructOperation::DrawLine(A3DVECTOR3 v1,A3DVECTOR3 v2, DWORD clr)
{
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
	if(pMap)
	{
...
void CObstructOperation::DrawObstruct()
{
	float fWidth = 1.0f;
	int   nShowTitle = 15;
				...
				for( int j = -nShowTitle; j < nShowTitle; j++)
				{
					...
					if( v>=0 && v < nTotal)
					{
						...
						if(g_Render.GetA3DEngine()->GetActiveCamera()->PointInViewFrustum(leftup))
						{
							...
							FIX16 idh= pMap->GetPassMapGen()->GetDeltaHeightMap()[(m_iObsMapWidth-v-1)* m_iObsMapWidth + h];
							//*********************************
							// revised by wenfeng, 06-1-13
							if(idh)
							{
								...
								pMap->GetPassMapGen()->Transf3DTo2D(vSamplePos, MapPos);
								pMap->GetPassMapGen()->Transf2DTo3D(MapPos, vPixelCenter);
...
void CObstructOperation::Release()
{
	if(m_pData) delete []m_pData;
...
void CObstructOperation::TransObstructToBitmap()
{
	CElementMap *pMap = AUX_GetMainFrame()->GetMap();
	if(pMap==NULL) return;
	UCHAR *pData = pMap->GetPassMapGen()->GetPassMap();	
