...
void CBitChunk::SetSize(int width,int height)
{
	if(width<=0 || height<=0)
	{
		AfxMessageBox("CBitChunk::SetSize(),内存分配没有成功！内存块的宽和高必须大于0");
		m_nWidth = 0;
		m_nHeight = 0;
		return;
	}
	...
	{
		m_nByteNum = 0;
		AfxMessageBox("CBitChunk::SetSize(),内存分配没有成功！可能是内存已经耗尽，请关闭一些其他应用程序");
		return;
	}
	//全部初始化为0
...
void CBitChunk::SetValue( int w, int h, int value)
{
	...
	{
		AfxMessageBox("CBitChunk::SetVaule(),超出取值范围");
		ASSERT(false);
	}
	//首先找出存放的字节
...
int CBitChunk::GetVaule( int w, int h)
{
	...
	if(w>=m_nWidth || w<0 || h>=m_nHeight || h<0)
	{
		AfxMessageBox("CBitChunk::GetVaule(),超出取值范围");
		ASSERT(false);
	}
	//首先找出存放的字节
	int pos = (m_nWidth*h + w)/8;
