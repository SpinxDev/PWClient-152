...
typedef DWORD (__stdcall* TP_CtfAImmActivate)(HMODULE* hMod);
typedef DWORD (__stdcall* TP_CtfAImmDeactivate)(HMODULE* hMod);
TP_CtfAImmActivate  pCtfAImmActivate = NULL;//(TP_CtfAImmActivate)GetProcAddress(GetModuleHandleA("imm32.dll"),"CtfAImmActivate");
TP_CtfAImmDeactivate  pCtfAImmDeactivate = NULL;//(TP_CtfAImmDeactivate)GetProcAddress(GetModuleHandleA("imm32.dll"),"CtfAImmDeactivate");
...
public:
	...
	void Initialize()
	{
		if(!pCtfAImmDeactivate)
		{
			//XP 系统tsf支持不好，关闭特定的tsf功能，强制输入法走imm流程
			pCtfAImmActivate = (TP_CtfAImmActivate)GetProcAddress(GetModuleHandleA("imm32.dll"),"CtfAImmActivate");
			pCtfAImmDeactivate = (TP_CtfAImmDeactivate)GetProcAddress(GetModuleHandleA("imm32.dll"),"CtfAImmDeactivate");
		}
	...
	void DisableCiceroOnThisWnd( HWND hwnd )
	{
		if(pCtfAImmDeactivate)
		{
			HMODULE hModuleMsctfime = GetModuleHandle(NULL);
			pCtfAImmDeactivate(&hModuleMsctfime);
		}
		if ( m_ptim == NULL )
			return;
...
static bool g_bCurrentTSF = false;

DWORD g_dwTSFThreadId = 0;
// 圣斗士使用, 给外部一个不使用ITfThreadMgrEx的接口
// win8有一些输入法不显示备选框问题
bool  g_bDisableITfThreadMgrEx = false;
...
public:
	...
	class CUIElementSink : public ITfUIElementSink, public ITfInputProcessorProfileActivationSink, public ITfCompartmentEventSink , public ITfActiveLanguageProfileNotifySink 
	{
	public:
		CUIElementSink();
		~CUIElementSink();
	...
	static ITfThreadMgrEx* m_tmEx;
	static ITfThreadMgr* m_tm;
	static ITfInputProcessorProfiles*  m_profiles;
	...
	static DWORD m_cookieActiveLPNotify;
...
ITfThreadMgr* CTsfUiLessMode::m_tm = NULL;
ITfThreadMgrEx* CTsfUiLessMode::m_tmEx  = NULL;
ITfInputProcessorProfiles*  CTsfUiLessMode::m_profiles = NULL;

DWORD CTsfUiLessMode::m_cookieActiveLPNotify = TF_INVALID_COOKIE;
...
#else
			wcscpy(szTemp, g_szReadingString);
#endif
...
#else
					wcscpy(g_szCompositionString, szCompStr);
#endif
					_SendCompString();
					InitCompStringData();
				}
...
#else
									wcscat(g_szReadingString, pszTmp);
#endif
								}
							}
						}
    ...
    case WM_IME_NOTIFY:
        switch (wParam)
        {
			...
			case IMN_SETOPENSTATUS:
				if (g_bUILessMode&&g_bCurrentTSF)
					break;
				CheckToggleState();
				break;
...
BOOL CTsfUiLessMode::SetupSinks()
{
	...
	hr = CoCreateInstance(CLSID_TF_InputProcessorProfiles, 
		NULL, 
		CLSCTX_INPROC_SERVER, 
		__uuidof(ITfInputProcessorProfiles), 
		(LPVOID*)&m_profiles);
	if (hr != S_OK)
	{
	...
	if(m_tmEx && !g_bDisableITfThreadMgrEx)
	{
		if (FAILED(m_tmEx->ActivateEx(&cid, TF_TMAE_UIELEMENTENABLEDONLY)))
		{
			//return FALSE;
		}
		else
	...
	if (m_TsfSink)
	{
		...
		{
			hr = srcTm->AdviseSink(__uuidof(ITfActiveLanguageProfileNotifySink),(ITfUIElementSink*)m_TsfSink,&m_cookieActiveLPNotify);
...
void CTsfUiLessMode::ReleaseSinks()
{
	...
	{
		if(TF_INVALID_COOKIE!=m_dwAlpnSinkCookie)
		{
		...
		if(TF_INVALID_COOKIE!=m_dwUIElementSinkCookie)
		{
			hr = source->UnadviseSink(m_dwUIElementSinkCookie);
			m_dwUIElementSinkCookie = TF_INVALID_COOKIE;
		}

		if(TF_INVALID_COOKIE!=m_cookieActiveLPNotify)
		{
			hr = source->UnadviseSink(m_cookieActiveLPNotify);
			m_cookieActiveLPNotify = TF_INVALID_COOKIE;
		}
		source->Release();
...
GUID  g_GuidUSKeyboard = {0x08040804,0x0000,0x0000, {0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00}};
STDMETHODIMP CTsfUiLessMode::CUIElementSink::OnActivated( REFCLSID clsid,REFGUID guidProfile, BOOL fActivated)
{
	extern DWORD g_dwTSFThreadId;
	if (g_dwTSFThreadId != GetCurrentThreadId())
	{
	...
	//if(fActivated)
	//{
	//	if((clsid==CLSID_NULL)&&(guidProfile==GUID_NULL))
	//	{
	//		HKL hkl;
	//		hkl = GetKeyboardLayout(0);
	//		memset(_xime_name,0,sizeof(_xime_name));
	//		ImmGetDescriptionW(hkl,_xime_name,sizeof(_xime_name) );
	//		g_bCurrentTSF = false;
	//		return S_OK;
	//	}
	...
	if(fActivated)
	{
		if(m_profiles)
		{
			...
			LANGID langID; 
			LANGID activeLangID;   
			GUID activeGuidProfile; 
			CLSID textServiceCLSID;   
			GUID xguidProfile;  
			hr = m_profiles->GetCurrentLanguage(&langID);  
			if (SUCCEEDED(hr))  
			{   
				if (langID!=0x0804)   
				{   
				...
				hr = m_profiles->GetDefaultLanguageProfile(langID, GUID_TFCAT_TIP_KEYBOARD, &textServiceCLSID, &xguidProfile); 

				//hr = m_profiles->GetActiveLanguageProfile(textServiceCLSID, &activeLangID, &activeGuidProfile);  
				if (SUCCEEDED(hr))  
				{    
					BSTR bstrDest;
					if (activeGuidProfile == g_GuidUSKeyboard)   
					{     
						//US keyboard     
						g_bChinese = false;      
					} 
					...
					{
						hr = m_profiles->GetLanguageProfileDescription(textServiceCLSID, langID, xguidProfile, &bstrDest);
						if(SUCCEEDED(hr))
						{
							memset(_xime_name,0,sizeof(_xime_name));
							wcscpy(_xime_name,bstrDest);
							g_bCurrentTSF = true;
						}
						else
						{
							...
							HKL hkl = NULL;
							UINT len = 0;
							if((textServiceCLSID==GUID_NULL)&&(xguidProfile!=GUID_NULL))
							{
								hkl = (HKL)xguidProfile.Data1;
								memset(_xime_name,0,sizeof(_xime_name));
								len = ImmGetDescriptionW(hkl,_xime_name,sizeof(_xime_name) );
								g_bCurrentTSF = false;
							}
							if(len==0)
							{
			...
			//if(g_bChinese)
			//{
			//	ITfInputProcessorProfileSubstituteLayout *TfInputProcessorProfileSubstituteLayout;
			//	hr = m_profiles->QueryInterface(IID_ITfInputProcessorProfileSubstituteLayout,(LPVOID*)&TfInputProcessorProfileSubstituteLayout);
			//	if(SUCCEEDED(hr))
			//	{
			//		TfInputProcessorProfileSubstituteLayout->GetSubstituteKeyboardLayout(textServiceCLSID,activeLangID,activeGuidProfile,&hkl);
			//		TfInputProcessorProfileSubstituteLayout->Release();
			//	}
			//	else
			//	{
			//		hkl = GetKeyboardLayout(0);
			...
			//	hkl = GetKeyboardLayout(0);
			//}
			////PostMessage(GetActiveWindow(),WM_INPUTLANGCHANGE,(WPARAM)0,(LPARAM)hkl);
			////DWORD tid;
...
#else
			wcscpy( g_szReadingString, szStr );
#endif
#else
#if _MSC_VER >= 1300
			wcscpy_s( g_szReadingString, COUNTOF(g_szReadingString), bstr );
#else
			wcscpy( g_szReadingString, bstr );
#endif
#endif
			g_dwCount = cchMax;
...
DWORD ImeUi_GetPageCount()
{
	return g_uCandPageSize;
}

void ImeUi_SetupSink()
{
	CTsfUiLessMode::SetupSinks();
}
void ImeUi_ReleaseSink()
{
	CTsfUiLessMode::ReleaseSinks();
}

void    SetUiImeThreadID(DWORD tid)
{
	g_dwTSFThreadId = tid;
}