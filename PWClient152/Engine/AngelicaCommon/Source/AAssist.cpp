...
static const DWORD l_aCRC32Table[] =
{
...
DWORD a_MakeIDFromString(const char* szStr)
{
	...
	DWORD c = 0xffffffff; /* shift register contents */
	const BYTE* p = (const BYTE*)szStr;
...
AWString AS2WC(const char* szAnsi)
{
	...
	int iLen = a_MultiByteToWideChar(szAnsi, -1, NULL, 0);
	wchar_t* pszWide = new wchar_t [iLen];
	a_MultiByteToWideChar(szAnsi, -1, pszWide, iLen);

	AWString strWide(pszWide);
...
AString WC2AS(const wchar_t* szWide)
{
	...
	int iLen = a_WideCharToMultiByte(szWide, -1, NULL, 0);
	char* pszAnsi = new char [iLen];
	a_WideCharToMultiByte(szWide, -1, pszAnsi, iLen);
...
{
	return MultiByteToWideChar(iCodePage, 0, szMultiByte, iMultiByte, wszWideChar, iWideChar);
}

int a_WideCharToMultiByte(int iCodePage, const wchar_t* wszWideChar, int iWideChar, char* szMultiByte, int iMultiByte)
{
	return WideCharToMultiByte(iCodePage, 0, wszWideChar, iWideChar, szMultiByte, iMultiByte, NULL, NULL);
}
...
AString WC2AS(int iCodePage, const wchar_t* szWide)
{
	...
	int iLen = a_WideCharToMultiByte(iCodePage, szWide, -1, NULL, 0);
	char* pszAnsi = new char [iLen];
	a_WideCharToMultiByte(iCodePage, szWide, -1, pszAnsi, iLen);
...
static DWORD l_dwMultiplier = 16807;    // (7 ** 5)
static DWORD l_dwModuler = 2147483647;  // (2 ** 31) - 1	
static DWORD l_dwSeed = 765983;
...
int a_Random()
{
	DWORD dwRandUniform = DWORD(((__int64)l_dwMultiplier * l_dwSeed) % (__int64)l_dwModuler);
