...
bool CECSceneBlock::Load(AFile* pWorldFile, AFile* pBSDFile, float sx, float sz, float fDist)
{
	...
	if (m_pScene && m_pScene->IsRandomMap())
	{
...
bool CECSceneBlock::LoadEffects(int iNumEffect, bool bSkipData)
{
	...
	for (int i=0; i < iNumEffect; i++)
	{
		DWORD dwID = pOnmtMan->LoadEffect(m_pScene, m_pWorldFile, m_pScene->GetCenterX() + m_fOffsetXForRandomMap, m_pScene->GetCenterZ()+ m_fOffsetZForRandomMap);
		if (!dwID)
			continue;
...
bool CECSceneBlock::LoadECModels(int iNumModel, bool bSkipData)
{
	...
	for (int i=0; i < iNumModel; i++)
	{
		DWORD dwID = pOnmtMan->LoadECModel(m_pWorldFile, m_pScene->GetCenterX() + m_fOffsetXForRandomMap, m_pScene->GetCenterZ()+m_fOffsetZForRandomMap);
		if (!dwID)
			continue;
...
bool CECSceneBlock::LoadSounds(int iNumSound, bool bSkipData)
{
	...
	for (int i=0; i < iNumSound; i++)
	{
		DWORD dwID = pOnmtMan->LoadSoundObject(m_pScene, m_pWorldFile, m_pScene->GetCenterX() + m_fOffsetXForRandomMap, m_pScene->GetCenterZ()+m_fOffsetZForRandomMap);
		if (!dwID)
			continue;
