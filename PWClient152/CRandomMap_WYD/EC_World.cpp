...
bool CECWorld::LoadWorld(const char* szFile, const A3DVECTOR3& vInitLoadPos)
{
	...
	if (g_pGame->GetGameRun()->GetGameState() == CECGameRun::GS_GAME && strResFile.Find("a21")>=0)
	{
		m_pRandomMapProc->DoProcess("a21");
		strResFile = "temp\\maps\\a21\\a21.trn2";
		szFile = "temp\\maps\\a21\\a21.ecwld";
		pExt = strrchr(szFile, '.');
	}
...
//		g_pGame->GetA3DDevice()->SetFillMode(A3DFILL_WIREFRAME);
		if (!m_pScene->m_Env.bUnderWater)
			m_pA3DTerrain->Render(pA3DViewport);
		else
			m_pA3DTerrain->RenderUnderWater(pA3DViewport, m_pA3DTerrainWater->GetCurCaustTexture(),
								m_pA3DTerrainWater->GetCaustDU(), m_pA3DTerrainWater->GetCaustDV());
//		g_pGame->GetA3DDevice()->SetFillMode(A3DFILL_SOLID);
	}
