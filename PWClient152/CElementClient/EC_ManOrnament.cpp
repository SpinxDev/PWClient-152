...
DWORD CECOrnamentMan::LoadEffect(CECScene* pScene, AFile* pFile, float fOffX, float fOffZ, int subTrn)
{
	DWORD dwRead;
	...
	//	Add to table
	if (m_pGameRun->GetWorld()->IsRandomMap())
		CECSceneBlock::GenerateObjectIDForRandomMap(subTrn,Data.dwExportID);
...
DWORD CECOrnamentMan::LoadECModel(AFile* pFile, float fOffX, float fOffZ,int subTrn)
{
	DWORD dwRead;
...
DWORD CECOrnamentMan::LoadSoundObject(CECScene* pScene, AFile* pFile, float fOffX, float fOffZ,int subTrn)
{
	DWORD dwRead;
