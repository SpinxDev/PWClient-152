...
CECIvtrArmor::CECIvtrArmor(int tid, int expire_date) : CECIvtrEquip(tid, expire_date)
{
	m_iCID	= ICID_ARMOR;
	...
	DATA_TYPE DataType;
	m_pDBEssence	= (ARMOR_ESSENCE*)pDB->get_data_ptr(tid, ID_SPACE_ESSENCE, DataType);
	m_pDBMajorType	= (ARMOR_MAJOR_TYPE*)pDB->get_data_ptr(m_pDBEssence->id_major_type, ID_SPACE_ESSENCE, DataType);
	m_pDBSubType	= (ARMOR_SUB_TYPE*)pDB->get_data_ptr(m_pDBEssence->id_sub_type, ID_SPACE_ESSENCE, DataType);
...
CECIvtrArmor::CECIvtrArmor(const CECIvtrArmor& s) : CECIvtrEquip(s)
{
...
CECIvtrArmor::~CECIvtrArmor()
{
}

//	Set item detail information
bool CECIvtrArmor::SetItemInfo(BYTE* pInfoData, int iDataLen)
{
	...
	try
	{
		...
		ASSERT(iEssenceSize == sizeof (IVTR_ESSENCE_ARMOR));
		...
		m_Essence = *(IVTR_ESSENCE_ARMOR*)dr.Read_Data(iEssenceSize);
	...
	{
		ASSERT(0);
		a_LogOutput(1, "CECIvtrArmor::SetItemInfo, data read error (%d)", e.GetType());
		return false;
	}
...
const char* CECIvtrArmor::GetIconFile()
{
	return m_pDBEssence->file_icon;
}

//	Get item name
const wchar_t* CECIvtrArmor::GetName()
{
	return m_pDBEssence->name;
}

const wchar_t* CECIvtrArmor::GetPreviewInfo()
{
	...
	//	+ HP
	if (m_Essence.hp_enhance - aPEEVals[PEEI_HP] + aRefines[REFINE_HP])
	{
		AddDescText(ITEMDESC_COL_WHITE, false, pDescTab->GetWideString(ITEMDESC_ADDHP));
		AddDescText(ITEMDESC_COL_WHITE, true, _AL(" %+d"), m_Essence.hp_enhance - aPEEVals[PEEI_HP] + aRefines[REFINE_HP]);
	}
	
	//	+ MP
	if (m_Essence.mp_enhance - aPEEVals[PEEI_MP])
	{
		AddDescText(ITEMDESC_COL_WHITE, false, pDescTab->GetWideString(ITEMDESC_ADDMP));
		AddDescText(ITEMDESC_COL_WHITE, true, _AL(" %+d"), m_Essence.mp_enhance - aPEEVals[PEEI_MP]);
	}
...
bool CECIvtrArmor::GetRefineEffectFor(ACString & strEffect, const RefineEffect &rhs)
{
	strEffect.Empty();
	if (!m_bNeedUpdate){
		CECStringTab* pDescTab = g_pGame->GetItemDesc();
		...
		case REFINE_HP:
			strEffect.Format(_AL("%s%s %d %s(+%d)")
				, rhs.GetClrAttribute()
				, pDescTab->GetWideString(ITEMDESC_ADDHP)
				, m_Essence.hp_enhance - rhs.m_aPEEVals[PEEI_HP] + rhs.m_aRefines[REFINE_HP] + rhs.GetIncEffect()
				, rhs.GetClrEffect()
				, rhs.GetIncEffect());
			break;
...
const wchar_t* CECIvtrArmor::GetNormalDesc(bool bRepair)
{
	if (m_bNeedUpdate)
		return NULL;
	...
	if (m_Essence.hp_enhance - aPEEVals[PEEI_HP] + aRefines[REFINE_HP])
	{
		...
		AddDescText(white, true, _AL(" %+d"), m_Essence.hp_enhance - aPEEVals[PEEI_HP] + aRefines[REFINE_HP]);
	}

	//	+ MP
	if (m_Essence.mp_enhance - aPEEVals[PEEI_MP])
	{
		AddDescText(white, false, pDescTab->GetWideString(ITEMDESC_ADDMP));
		AddDescText(white, true, _AL(" %+d"), m_Essence.mp_enhance - aPEEVals[PEEI_MP]);
	}
...
bool CECIvtrArmor::HasRandomProp()
{
	for (int i=0; i < m_aProps.GetSize(); i++)
	{
		const PROPERTY& prop = m_aProps[i];
		if (!prop.bEmbed && !prop.bEngraved && prop.iType == 336)
			return true;
	}
...
int CECIvtrArmor::GetRefineMaterialNum()
{
	return m_pDBEssence->material_need;
}

int CECIvtrArmor::GetRefineAddOn()
{
	return m_pDBEssence->levelup_addon;
}
//	Get drop model for shown
const char * CECIvtrArmor::GetDropModel()
{
	return m_pDBEssence->file_matter;
}

bool CECIvtrArmor::IsRare() const
{ 
	return CECIvtrEquip::IsRare() || m_pDBEssence->level >= 6;
}

int CECIvtrArmor::GetItemLevel() const
{
